...
import { Globe, Plus, MapPin, Trash2, Edit2, Search, X, Save, List, LayoutGrid } from 'lucide-react';
...
const CountryManagement: React.FC<CountryManagementProps> = ({ countries, onUpdate }) => {
  ...
  const deleteCity = (cityId: string) => { onUpdate(countries.map(c => c.id === selectedCountryId ? { ...c, cities: c.cities.filter(city => city.id !== cityId) } : c )); };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
          ...
          {countries.map((country) => (
            ...
            >
              ...
              <div className="pt-4 border-t border-gray-50 flex justify-between">
                <span className="text-[10px] font-black text-blue-600 group-hover:underline">عرض المدن..</span>
                <Trash2 size={16} className="text-red-400 hover:text-red-600" onClick={(e) => { e.stopPropagation(); deleteCountry(country.id); }} />
              </div>
            </div>
          ))}
          <div 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          <div className="md:col-span-1 space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setShowCountryModal(true)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-xl transition-all"><Plus size={20} /></button>
            </div>
              ...
              <div className="divide-y divide-gray-50 max-h-[500px] overflow-y-auto custom-scrollbar">
                {countries.map((country) => (
                  <div key={country.id} className="group flex items-center bg-white">
                    <button onClick={() => setSelectedCountryId(country.id)} className={`flex-1 text-right p-4 flex items-center justify-between transition-all ${selectedCountryId === country.id ? 'bg-blue-600 text-white' : 'hover:bg-gray-50 text-gray-600'}`}>
                    ...
                    <button onClick={() => deleteCountry(country.id)} className="p-4 text-red-400 hover:text-red-600 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all border-r border-gray-50"><Trash2 size={16} /></button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="flex items-center justify-between mb-2"><h4 className="font-black text-gray-800">{selectedCountry ? `المدن في ${selectedCountry.name}` : 'اختر دولة لعرض المدن'}</h4>{selectedCountry && ( <button onClick={() => setShowCityModal(true)} className="flex items-center gap-2 bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-xs font-black hover:bg-blue-100 transition-all"><Plus size={16} /> إضافة مدينة</button> )}</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 p-4 sm:p-6">
                {selectedCountry?.cities.map((city) => (
                  <div key={city.id} className="group flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-2xl border border-transparent hover:border-blue-200 hover:bg-white transition-all shadow-sm hover:shadow-md">
                    ...
                    <div className="flex gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity"><button className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 size={14} /></button><button onClick={() => deleteCity(city.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button></div>
                  </div>
                ))}
      ...
      {showCountryModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-5 border-b flex justify-between items-center bg-gray-50"><h4 className="text-lg font-black text-gray-800">إضافة دولة جديدة</h4><button onClick={() => setShowCountryModal(false)} className="p-2 hover:bg-gray-200 rounded-xl"><X size={20} /></button></div>
            <form onSubmit={handleAddCountry} className="p-5 space-y-4">
              <div className="space-y-1"><label className="text-xs font-bold text-gray-500">اسم الدولة</label><input required value={newCountry.name} onChange={e => setNewCountry({...newCountry, name: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" /></div>
              <div className="space-y-1"><label className="text-xs font-bold text-gray-500">الرمز (ISO)</label><input required value={newCountry.code} onChange={e => setNewCountry({...newCountry, code: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl outline-none uppercase text-left" maxLength={3} /></div>
              <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-xl font-black flex items-center justify-center gap-2 hover:bg-blue-700 transition-all shadow-lg active:scale-95"><Save size={18} /> حفظ</button>
            </form>
          </div>
        </div>
      )}
      {showCityModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-5 border-b flex justify-between items-center bg-gray-50"><h4 className="text-lg font-black text-gray-800 leading-tight">إضافة مدينة إلى {selectedCountry?.name}</h4><button onClick={() => setShowCityModal(false)} className="p-2 hover:bg-gray-200 rounded-xl"><X size={20} /></button></div>
            <form onSubmit={handleAddCity} className="p-5 space-y-4">
              <div className="space-y-1"><label className="text-xs font-bold text-gray-500">اسم المدينة</label><input required value={newCityName} onChange={e => setNewCityName(e.target.value)} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" autoFocus /></div>
              <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-xl font-black flex items-center justify-center gap-2 active:scale-95 shadow-lg"><Save size={18} /> إضافة</button>
            </form>
          </div>
        </div>
      )}
